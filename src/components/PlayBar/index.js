...
export default class PlayBar extends PureComponent {
  ...
  componentWillUnmount() {
    ...
    pubsub.unsubscribe(
      this.playSubscribeId,
      this.addSubscribeId,
      this.closeSubscribeId
    )
  }
  ...
  fetchSongUrl = (id) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      // TODO 切换音质
      requestSongResource({ id, level: 'higher' })
        .then((res) => {
  ...
  getPrevIndex = () => {
    ...
    let prevIndex = index
    if (mode === PLAY_MODE.LOOP || mode === PLAY_MODE.ORDER) {
      // 单曲循环和顺序播放
      prevIndex = index - 1
      if (prevIndex < 0) {
      ...
      () => {
        if (this.state.volumeVisible) {
          ...
          if (!this.volumeHeight) {
            this.volumeHeight = parseInt(
              window.getComputedStyle(this.volumeLineRef.current).height,
              10
            )
          }
          const volumeLength =
            this.props.player.playSetting.volume * this.volumeHeight
  ...
  setVolumeStyle = (volumeLength) => {
    ...
    if (!volumeLength) {
      ...
      volumeTop =
        this.volumeHeight - volumeLength - (this.volumeDotR - offset) * 2
    }
    if (volumeTop < -offset) {
      volumeTop = -offset
    }
  ...
  handleVolumeClick = (e) => {
    const boundingClientRect =
      this.volumeLineRef.current.getBoundingClientRect()
  ...
  getRenderMode = (mode) => {
    switch (mode) {
      case PLAY_MODE.LOOP:
        return (
            <a className={`${styles.icon} ${styles.loop}`} title="单曲循环">
              单曲循环
            </a>
        )
      case PLAY_MODE.SHUFFLE:
        return (
            <a className={`${styles.icon} ${styles.shuffle}`} title="随机">
              随机
            </a>
        )
      default:
        return (
            <a className={`${styles.icon} ${styles.order}`} title="列表循环">
              列表循环
            </a>
        )
    }
  ...
  render() {
    const { trackQueue, playSetting, isPlaying, currentPlayedTime } =
      this.props.player
    ...
    return (
      <div
        id={PLAY_BAR_DOM_ID}
        className={`${styles.wrapper} ${playSetting.isLocked ? styles.locked : styles.unlock}`}
        style={playBarStyle}
      ...
      >
          ...
          <div className={styles.progress}>
            <div className={styles['song-info']}>
              ...
              {Array.isArray(artists) ? (
                <div className={styles.singer} title={getArtists(artists)}>
              ...
              {radio ? (
                <div className={styles.singer}>
                  <Link to={`/radio/${radio.id}`} onClick={this.closePanel}>
                    {radio.name}
                  </Link>
                </div>
              ) : null}
            ...
            <div
              ...
              className={styles['progress-cont']}
            ...
            >
              ...
              >
                <span
                  className={`${styles.loading} ${loading ? styles.visible : styles.hidden}`}
                />
              </div>
            </div>
          </div>
          <div className={styles.time}>
            <em>{formatDuration(currentPlayedTime * 1000)}</em> /{' '}
            {formatDuration(song.duration)}
          </div>
          <div className={styles.operation}>
            <AddToPlaylist songIds={[id]}>
              <span
                className={`${styles.icon} ${styles.collect}`}
                title="收藏"
              />
            </AddToPlaylist>
            <span className={`${styles.icon} ${styles.share}`} title="分享" />
          </div>
          <div className={styles.setting}>
            <div className={styles.volume}>
              ...
              <div onClick={this.handleVolumeControl}>
                {playSetting.volume ? (
                  <span
                    className={`${styles.icon} ${styles['volume-icon']} ${styles['volume-sound']}`}
                  />
                ) : (
                  <span
                    className={`${styles.icon} ${styles['volume-icon']} ${styles['volume-mute']}`}
                  />
                )}
              </div>
              {/* 音量控制 */}
              ...
              >
                ...
                >
                  <div
                    className={styles['volume-current']}
                    style={{ height: volumeLength + 'px' }}
                  />
                  <span
                    ref={this.volumeDotRef}
                    style={volumeDotStyle}
                    className={styles['volume-dot']}
            ...
            {/* 播放模式 */}
            <div className={styles.mode} onClick={this.changeMode}>
              {this.getRenderMode(playSetting.mode)}
            </div>
            <div
              className={`${styles.icon} ${styles['playlist-icon']}`}
              onClick={this.handleSwitchPlayPanel}
            >
              <div
                style={{ display: playedTipVisible ? 'block' : 'none' }}
                className={styles['operation-tip']}
              >
                已开始播放
              </div>
              <div
                style={{ display: addedTipVisible ? 'block' : 'none' }}
                className={styles['operation-tip']}
              >
                已添加到播放列表
              </div>
        ...
        <div className={styles.lock}>
          <span className={styles['lock-icon']} onClick={this.handleLock} />
        </div>
        <PlayPanel
