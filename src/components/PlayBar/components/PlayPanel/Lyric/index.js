...
export default class Lyric extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      visible &&
      songId &&
      (songId !== prevProps.song.id || this.requestedSongId !== songId)
    ) {
  ...
  scrollToCurrentTime = () => {
    if (this.scrollbarRef.current) {
      if (this.formattedLyric) {
        const seconds = Object.keys(this.formattedLyric).map(
          (key) => this.formattedLyric[key].second
        )
        ...
        if (
          typeof activeTime === 'undefined' ||
          activeTime === this.prevActiveTime
        ) {
          return
        }

        const activeEle = document.querySelector(
          `.lyric-line[data-seconds="${activeTime}"]`
        )
        if (activeEle) {
          const activeEleHeight = activeEle.offsetHeight
          const offset =
            activeEle.offsetTop - (this.props.height - activeEleHeight) / 2
          const scrollTop = offset < 0 ? 0 : offset
          const currentScrollTop = this.scrollbarRef.current.getScrollTop()
          const duration = DURATION
          const intervalTime = 50
          const intervalHeight = Math.round(
            Math.abs((scrollTop - currentScrollTop) / (duration / intervalTime))
          )
  ...
  getRenderLyric = () => {
    ...
    if (song.program) {
      return <div className={styles['no-lyric']}>电台节目，无歌词</div>
    }
    if (!song.id) {
      return ''
    }
  ...
  getLyricElement = (lyric) => {
    ...
    convertedLyric.forEach((item, index) => {
      ...
      if (transform) {
        ...
        originLyrics.forEach((v, i) => {
          ...
          lyricElement.push(
            <p
              className={`${isActive && i === originLyrics.length - 1 ? styles.active : ''} ${styles['lyric-line']}`}
    ...
    if (!this.hasTime) {
      lyricElement.unshift(
        <p key="tip">
          *该歌词不支持自动滚动* <a>求滚动歌词</a>
        </p>
      )
    }
    return lyricElement
  ...
  render() {
    ...
    return (
      <div className={styles['lyric-wrapper']} style={{ height: height }}>
        <VerticalScrollbar ref={this.scrollbarRef}>
          <div className={styles.lyric}>{this.getRenderLyric()}</div>
        </VerticalScrollbar>
      </div>
    )
  }
