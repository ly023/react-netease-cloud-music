    ...
    if (this.scrollbarRef.current) {
      ...
      if (index === 0) {
        ...
        const currentPageEndIndex =
          Math.floor((currentScrollTop + height) / ITEM_HEIGHT) - 1

        // 如果index在可见范围内，就不进行定位
        if (index <= currentPageEndIndex && index >= currentPageStartIndex) {
          return
        }
        if (index <= currentPageStartIndex - 1) {
          // 上
          scrollTop = ITEM_HEIGHT * index
        } else if (index >= currentPageEndIndex + 1) {
          // 下
            ...
            <ul>
              {trackQueue.map((item, idx) => {
                ...
                return (
                  <li
                    key={`${id}-${idx}`}
                    className={`${styles.item} ${idx === index ? styles.active : ''}`}
                    onClick={() => this.handlePlay(idx)}
                  >
                    <div className={styles['arrow-play-icon']} />
                    <div className={styles.name}>{item.name}</div>
                    <div className={styles.operation}>
                      <span
                        className={`${styles.icon} ${styles['delete-icon']}`}
                    ...
                    <span className={styles.link} onClick={this.handleRedirect}>
                      <Link to="/">来源</Link>
                    </span>
                  </li>
                )
