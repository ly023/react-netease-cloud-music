...
function CreatePlaylistModal(props) {
    ...
    const validateName = useCallback((rule, value, callback) => {
        if (!trim(value) && !(inputRef.current && inputRef.current === document.activeElement)) {
            callback('歌单名不能为空')
            return
        }
        const maxBytes = 40
    ...
    >
        <div styleName="form">
            ...
            {error ? <div styleName="error"><span styleName="error-icon"/>{error}</div> : null}
            <p styleName="tip">可通过“收藏”将音乐添加到新歌单中</p>
            <div styleName="footer">
                <CustomButton type="primary" disabled={loading} onClick={handleSubmit}>{loading ? '新建中...' : '新 建'}</CustomButton>
