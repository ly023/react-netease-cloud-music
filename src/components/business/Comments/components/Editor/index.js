...
export default class Editor extends Component {
  ...
  setInputValue = (value, callback) => {
    ...
    const doubleByteChar = value.match(
      new RegExp(DOUBLE_BYTE_CHAR_PATTERN, 'g')
    )
    ...
    const remainingWordsNumber =
      MAX_WORDS_NUMBER - (charNumber + doubleByteCharNumber)
    ...
    if (value) {
      ...
      this.setState(
        ...
        () => {
          ...
          const hasScrollbar =
            this.inputRef.current.scrollHeight !==
          ...
          if (
            /^[a-zA-Z0-9_\-一-龥]{1,15}$/.test(atText) ||
            (atCursorStart && atCursorStart === cursorStart)
          ) {
    ...
    if (val) {
      ...
      this.setInputValue(newValue, () => {
        this.setSelectionRange(
          cursorStart + insertedLen,
          cursorEnd + insertedLen
        )
      })
    }
  }
    ...
    return (
      <div className={className}>
        <div className={styles['text-wrapper']}>
          ...
          {/* 用来计算光标的位置 -- start */}
          <div
            className={`${styles.textarea} ${styles.display}`}
            ref={this.displayRef}
            style={displayStyle}
          >
            {value.substr(0, atCursorStart)}
            <span className={styles.mark} ref={this.markRef} />
          </div>
          {/* 用来计算光标的位置 -- end */}
          <AtList
        ...
        <div className={`clearfix ${styles.operation}`}>
          <TagFacesIcon
            id="comment-emoji-panel-icon"
            className={styles.icon}
            onClick={this.showEmojiPanel}
          />
          <AlternateEmailIcon
            id="comment-at-list-icon"
            className={styles.icon}
            onClick={this.showAtList}
          />
          <a
          ...
          <span
            className={`${styles.count}${remainingWordsNumber < 0 ? styles.error : ''}`}
          >
            {remainingWordsNumber}
          </span>
          <EmojiPanel
