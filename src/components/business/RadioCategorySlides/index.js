...
function RadioCategorySlides(props) {
  ...
  return (
    <div className="categories">
        ...
        {slides.map((item) => {
          ...
          return (
            <Link
              key={id}
              to={categoryLink}
              className={`item ${categoryId === id ? 'active' : ''}`}
            >
              <div
                className="icon"
      ...
      <div className="pagination">
        {pagination.map((v, index) => {
          return (
            <span
              key={index}
              className={`dot ${index === activePageIndex ? 'active' : ''}`}
      ...
      <div
        className={`prev ${activePageIndex === 0 ? 'disabled' : ''}`}
        onClick={handlePrev}
      />
      <div
        className={`next ${activePageIndex === pagination.length - 1 ? 'disabled' : ''}`}
        onClick={handleNext}
      />
    </div>
  )
