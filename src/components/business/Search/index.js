...
export default class Search extends Component {
  ...
  handleSearch = () => {
    ...
    if (keyword !== searchValue && keyword && keyword.trim()) {
      ...
      this.props.navigate(
        `/search?s=${window.encodeURIComponent(keyword)}&type=${type}`
      )
  ...
  fetchSearchSuggest = (value) => {
    ...
    requestSearchSuggest(params).then((res) => {
  ...
  getClass = (type, item) => {
    return `/${type.substr(0, type.length - 1)}/${item.id}` ===
  ...
  getRenderSongItem = (item) => {
    return (
      <Link to={`/song/${item.id}`}>
        {this.getRenderKeyword(`${item.name}-${getArtists(item.artists)}`)}
      </Link>
    )
  }
  ...
  getRenderArtistItem = (item) => {
    return (
      <Link to={`/artist/${item.id}`}>{this.getRenderKeyword(item.name)}</Link>
    )
  }
  ...
  getRenderAlbumItem = (item) => {
    return (
      <Link to={`/album/${item.id}`}>
        {this.getRenderKeyword(`${item.name}-${item.artist?.name}`)}
      </Link>
    )
  }
  ...
  getRenderMVItem = (item) => {
    ...
    return (
      <Link to={pathname}>
        MV:{this.getRenderKeyword(`${item.name}-${item.artistName}`)}
      </Link>
    )
  }
  ...
  getPageSearchInput = () => {
    return (
      <div className={styles['page-search-bar']}>
        <input
          ref={this.inputRef}
        ...
        />
        <span
          className={styles['page-search-icon']}
  ...
  render() {
    ...
    return (
      <div id={wrapperId} className={styles.wrapper}>
        <div
          className={styles['mv-support']}
          style={{ display: !keyword && mvSupportVisible ? 'block' : 'none' }}
        >
          <p>现在支持搜索MV啦</p>
        </div>
        ...
        <div
          className={
            type === 'navSearch'
              ? styles['nav-search-layout']
              : styles['page-search-layout']
          }
          ...
          <div className={styles.layout}>
            <div className={styles.list}>
              <p className={styles.note}>
                <Link
                  to={searchUrl}
                  className={searchUrl === activeUrl ? styles.active : ''}
                  ...
                  {Object.keys(result).map((type, index) => {
                    ...
                    return (
                      <div key={type} className={styles.item}>
                        ...
                        <ul
                          className={`${styles.result}${index % 2 ? styles.odd : ''}`}
                          onClick={this.hideResult}
                        >
                          {values.map((v) => {
                            return (
                              <li key={v.id} className={this.getClass(type, v)}>
