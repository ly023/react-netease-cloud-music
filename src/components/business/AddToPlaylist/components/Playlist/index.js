...
const Row = (props) => {
  ...
  if (!id) {
    return <div style={style}>{extraItem}</div>
  }
  ...
  return (
    <div
      ...
      onClick={(e) => itemClick(e, id)}
    >
      <div className={styles.box}>
        <div className={styles.left}>
          <img
            src={getThumbnail(item.coverImgUrl, 40)}
        ...
        <div className={styles.meta}>
          <p className={styles.name}>
            ...
            item.specialType === DefaultPlaylistType
              ? '我喜欢的音乐'
              : name}
          </p>
          {item.trackCount ? (
            <p className={styles['track-count']}>{item.trackCount}首</p>
          ) : null}
        </div>
      </div>
    </div>
