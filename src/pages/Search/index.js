...
export default class SearchPage extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const searchValue = getUrlParameter('s')
    const { activeType: prevActiveType, searchValue: prevSearchValue } =
      prevState
  ...
  fetchData = (offset = 0) => {
    ...
    requestMultiMatch(params).then((res) => {
      if (this._isMounted) {
        const { result = {} } = res
        this.setState({
      ...
      case SEARCH_TYPE.PLAYLIST.TYPE:
        return (
          <Playlists
            keyword={searchValue}
            list={list}
            onSubscribeSuccess={this.handleSubscribePlaylistSuccess}
          />
        )
      case SEARCH_TYPE.USER.TYPE:
                      ...
                      {recQuery.map((s) => {
                        return (
                          <Link
                            ...
                            to={`/search?s=${window.encodeURIComponent(s)}&type=${activeType}`}
                          >
                            {s}
                          </Link>
                        )
